...
\begin{itemize}
	\item \textbf{H1. 单一指派约束。} 每个护士每天只能被安排到一个班次并使用一种技能。在本文提出的解向量的表示方式下，该硬约束将自动满足。
	
	\item \textbf{H2. 人数下限约束。} 每天每班次每技能的排班人数不能小于其人数需求的最小值。
	...
	\item \textbf{H4. 技能约束。} 安排到某个时间槽的护士必须具备对应的技能。
\end{itemize}

此外，还有8个软约束：

\begin{itemize}
	\item \textbf{S1. 最优人数约束。} 每个时间槽安排的护士数量不得小于给定的最优值。每缺少一个护士都要受到30个单位的惩罚。
	
	\item \textbf{S2. 连续工作日约束。} 每个护士的连续工作天数必须在给定的区间内，每个超出或不足的工作日都要受到30个单位的惩罚。
	
	\item \textbf{S3. 连续相同班次约束。} 每个护士被连续安排到同一班次的天数必须在给定的区间内，每个超出或不足的工作日都要受到15个单位的惩罚。
	
	\item \textbf{S4. 连续休息约束。} 每个护士的连续休息天数必须在给定的区间内，每个超出或不足的休息日都要受到30个单位的惩罚。
	
	\item \textbf{S5. 工作偏好约束。} 如果一个护士请求不在某天某班次上班，但仍然被安排在该天该班次上班，将受到10个单位的惩罚。
	
	\item \textbf{S6. 完整周末约束。} 如果一个护士的合同规定其应有完整的周末，则其在周末的排班必须是两天均上班或者两天均休息，否则将受到30个单位的惩罚。
	
	\item \textbf{S7. 总排班数约束。} 每个护士在整个排班周期中的总工作天数必须在给定的区间内，每个超出或不足的工作日都要受到20个单位的惩罚。
	
	\item \textbf{S8. 总周末排班数约束。} 每个护士在整个排班周期中被排班了的周末总数不得大于给定的最大值，每超出一个周末都要受到30个单位的惩罚。
\end{itemize}

其中\textbf{S7}与\textbf{S8}为横跨整个排版周期的约束，而其他约束可以在各阶段完成计算。
