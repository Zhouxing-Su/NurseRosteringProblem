...
namespace NurseRostering
{
    ...
    public class Problem
    {
        ...
        public void readCustomFile(string path) {
            history = Util.readJsonFile<HistoryInfo>(path);
        }

        public void writeCustomFile(string path, Solver solver) {
            Util.writeJsonFile(path, solver.generateHistory());
        }
    ...
    public abstract class Solver
    {
        ...
        public virtual void record(string logPath) {
            ...
            using (Mutex fileLock = new Mutex(false, "NurseRostoring.Solver.LogFileLock")) {
                fileLock.WaitOne();
        ...
        /// <summary> operations like removing spanning constraints. </summary>
        public abstract void adjustObjValueForLog();
        ...
        protected void errorLog(string msg) {
            Util.printLog(environment.id + "," + msg);
        }
        #endregion Method
    ...
    public class TabuSolver : Solver
    {
        ...
        #region Method
        public override void adjustObjValueForLog() {
            if (H.currentWeek < S.totalWeekNum) {
        ...
        protected class Solution : Output
        {
            ...
            public Problem.HistoryInfo generateHistory() {
            ...
            /// reaches maxNoImproveForAllNeighborhood. for each neighborhood week, the possibility          <para />
            /// to select is P[week]. increase the possibility to select when no improvement. in detail,     <para />
            /// the P[week] contains two part, local and global. the local part will increase if the         <para />
            /// neighborhood week makes improvement, and decrease vice versa. the global part will           <para />
            /// increase if recent search (not only on neighborhood week) can not make improvement,          <para />
